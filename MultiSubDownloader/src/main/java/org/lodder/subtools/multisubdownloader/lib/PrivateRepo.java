...
public class PrivateRepo {
  ...
  public List<Subtitle> searchSubtitles(EpisodeFile episodeFile, String languageCode)
    ...
    for (IndexSubtitle indexSubtitle : index) {
      if (indexSubtitle.getTvdbid() == episodeFile.getTvdbid()) {
        if (indexSubtitle.getSeason() == episodeFile.getSeason()
            && indexSubtitle.getEpisode() == episodeFile.getEpisodeNumbers().get(0)) {
          if (indexSubtitle.getLanguage().equalsIgnoreCase(languageCode)) {
            ...
            Subtitle tempSub =
                new Subtitle(Subtitle.SubtitleSource.PRIVATEREPO, indexSubtitle.getFilename(),
                    location, indexSubtitle.getLanguage(), "", SubtitleMatchType.EVERYTHING,
                    VideoFileParser.extractTeam(indexSubtitle.getFilename()), "", false);
            results.add(tempSub);
          }
        }
      }
